...
def add_locations_to_room(
    world: World, room: Room, loc_category: str, count: int = 5, random_seed: int = 42
):
...
def add_fossils_to_room(
    world: World, room: Room, count: int = 5, random_seed: int = 42
):
    obj_category = "fossil"
    ...
    fossil_sites = add_locations_to_room(
        world, room, "fossil_site_box", random_seed=random_seed
    )
...
def create_fossil_world(
    ...
    room = world.add_room(
        name="exploration_zone", footprint=exploration_coords, color=[0.8, 0.8, 0.8]
    )
    ...
    add_fossils_to_room(world, room, count=n_fossils, random_seed=random_seed + 1 * 42)
    add_locations_to_room(
        world, room, "rock", count=n_rocks, random_seed=random_seed + 2 * 42
    )
    add_locations_to_room(
