...
class ExplorerBatteryManager:
    ...
    def update_battery(self, charge_rate: float = 10.0):
        if self.is_at_charger():
            if not self.charging:
                ...
                if charger and self.charging_coordinator.start_charging(
                    charger.name, self.robot.name
                ):
                    self.charging = True
                    self.is_seeking_charger = False
            ...
            if self.charging:
                ...
                if new_charge <= self.last_charge_update:
                    self.logger.warning(
                        "Battery not charging properly, resetting charge state"
                    )
                    self.charging = False
                    self.current_charger = None
    ...
    def validate_stored_path(self) -> bool:
        ...
        try:
            new_path = self.robot.path_planner.plan(
                current_pose, self.on_charge_finished_path.poses[-1]
            )
            if new_path is None:
    ...
    def resume_after_charging(self) -> bool:
        try:
            ...
            while self.path_execution_retries < self.MAX_PATH_RETRIES:
                ...
                self.logger.warning(
                    f"Path execution retry {self.path_execution_retries}"
                )
    ...
    def replan_path(self) -> bool:
        if self.pre_charge_position:
            try:
                new_path = self.robot.path_planner.plan(
                    self.robot.get_pose(), self.pre_charge_position
                )
                if new_path:
    ...
    def needs_charging(self) -> bool:
        ...
        if self.battery.charge <= self.MIN_SAFE_BATTERY:
            path_to_charger, charger = (
                self.charging_coordinator.get_best_available_charger(
                    self.robot.get_pose(), self.world, self.robot.path_planner
                )
            )

            if path_to_charger is not None:
                if self.charging_coordinator.reserve_charger(
                    charger.name, self.robot.name
                ):
    ...
    def follow_path_with_drain(self, path: Path) -> bool:
        try:
            ...
            if result:
                ...
                scan_drain = self.battery.actionDrains.get("scan", 0) * (
                    path.num_poses / 2
                )
    ...
    def execute_path_safely(self, goal: Pose) -> bool:
        ...
        if self.needs_charging():
            ...
            if (
                path_to_charger is not None and path_to_charger.poses
            ):  # Check if path has poses
                if self.charging_coordinator.reserve_charger(
                    charger.name, self.robot.name
                ):
                    ...
                    try:
                        ...
                        if path_from_charger is not None and path_from_charger.poses:
                            ...
                            return_path = self.robot.path_planner.plan(
                                path_to_charger.poses[-1], self.pre_charge_position
                            )
                            if return_path is not None and return_path.poses:
                                self.on_charge_finished_path = return_path
                                self.logger.info(
                                    "Will return to pre-charge position after charging"
                                )
                            else:
                                self.logger.info(
                                    "No valid path after charging, will resume exploration"
                                )
            ...
            if self.reserved_charger:
                try:
                    ...
                    if path_from_charger is None or not path_from_charger.poses:
                        self.charging_coordinator.stop_charging(
                            self.reserved_charger.name, self.robot.name
                        )
                        self.reserved_charger = None
                        return False
                    self.on_charge_finished_path = path_from_charger
                    ...
                    self.logger.error(
                        f"Error planning path from charger to goal: {str(e)}"
                    )
                    return False
    ...
    def plan_path_through_charger(self, goal: Pose) -> tuple[bool, Path]:
        try:
            ...
            path_to_charger, charger = (
                self.charging_coordinator.get_best_available_charger(
                    self.robot.get_pose(), self.world, self.robot.path_planner, goal
                )
            )

            if (
                path_to_charger is None or not path_to_charger.poses or charger is None
            ):  # Check if path has poses
