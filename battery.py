...
class Battery:
    ...
    def completeDrainForPath(self, path: list[Pose], startingPose: Pose, actions: list[str] = None):
        ...
        while (stepCount<len(path)-1 and workingCharge >= 0):
            currentPose = path[stepCount]
            workingCharge -= currentPose.get_linear_distance(path[stepCount + 1]) * self.drainPerDistanceUnit
            workingCharge -= abs(currentPose.get_angular_distance(path[stepCount + 1]) * self.drainPerRadianRotate)
            stepCount += 1
    ...
    def get_optimal_charger(self, start: Pose, world: World, planner: RRTPlanner, goal: Pose = None) -> tuple[Path, Location]:
    ...
    def get_drain_for_path(self, path: Path) -> float:
        ...
        for i in range(path.num_poses - 1):
            ...
            linear_drain = current_pose.get_linear_distance(next_pose) * self.drainPerDistanceUnit
            angular_drain = abs(current_pose.get_angular_distance(next_pose) * self.drainPerRadianRotate)
